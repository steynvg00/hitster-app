<script lang="ts">
	...
	type FormResult = { error?: string; sent?: boolean; email?: string } | null;
...
{#if form?.sent}
	<div class="rounded-lg border border-green-800 bg-green-950 px-4 py-3 text-sm text-green-300">
		Magic link sent to <strong>{form.email}</strong>. Check your inbox.
	</div>
{:else if !showEmail}
	<!-- Google sign-in -->
	<form
...
{:else}
	...
	>
		<div>
			<label for="host-email" class="mb-2 block text-xs uppercase tracking-widest text-zinc-400">
				Email
			</label>
			<input
				id="host-email"
				...
				autocomplete="email"
				required
				class="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-3 text-white placeholder-zinc-500 focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400"
