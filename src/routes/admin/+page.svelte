<script lang="ts">
	...
	function fireFlame(side: 'left' | 'right' | 'both') {
		if (side === 'left' || side === 'both') {
			flameLeft = false;
			requestAnimationFrame(() => { flameLeft = true; });
			setTimeout(() => { flameLeft = false; }, 1400);
		}
		if (side === 'right' || side === 'both') {
			flameRight = false;
			requestAnimationFrame(() => { flameRight = true; });
			setTimeout(() => { flameRight = false; }, 1400);
		}
	}
	...
	$effect(() => {
		...
		if (prev === 'playing' && ps === 'recap') {
			...
			if (c) {
				setTimeout(() => { fireConfetti(c, 0.35, 0.25, 50); }, 100);
				setTimeout(() => { fireConfetti(c, 0.65, 0.25, 50); }, 300);
				setTimeout(() => { fireConfetti(c, 0.5, 0.2, 30); }, 700);
			}
		}
	});
	...
	onMount(() => {
		...
		if (!data.activeSet) return () => { clearInterval(clockIv); clearTimeout(flameTimer); clearTimeout(confettiTimer); };
	...
	<div class="theme-bg" aria-hidden="true">
		...
		{:else if themeStore.current === 'showtime'}
			...
			<div class="showtime-stage">
				<svg viewBox="0 0 1200 700" preserveAspectRatio="xMidYMin slice" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
					<defs>
					...
					<!-- Speaker stacks left -->
					<rect x="50" y="200" width="90" height="340" fill="#111" rx="3" stroke="#222" stroke-width="1"/>
					<rect x="60" y="210" width="70" height="310" fill="#0a0a0a" rx="2"/>
					{#each Array.from({ length: 8 }, (_, i) => i) as i}
						<circle cx="95" cy={230 + i * 36} r="12" fill="#0d0d0d" stroke="#1e1e1e" stroke-width="1"/>
					{/each}
					<!-- Speaker stacks right -->
					<rect x="1060" y="200" width="90" height="340" fill="#111" rx="3" stroke="#222" stroke-width="1"/>
					<rect x="1070" y="210" width="70" height="310" fill="#0a0a0a" rx="2"/>
					{#each Array.from({ length: 8 }, (_, i) => i) as i}
						<circle cx="1105" cy={230 + i * 36} r="12" fill="#0d0d0d" stroke="#1e1e1e" stroke-width="1"/>
					{/each}
					<!-- Side LED columns left -->
					<rect x="155" y="80" width="60" height="500" fill="#0a0a0a" rx="2" class="showtime-led-glow" stroke="#1a1a1a" stroke-width="2"/>
					<!-- Side LED columns right -->
					<rect x="985" y="80" width="60" height="500" fill="#0a0a0a" rx="2" class="showtime-led-glow" stroke="#1a1a1a" stroke-width="2"/>
					<!-- Center LED wall frame -->
					<rect x="230" y="70" width="740" height="490" fill="url(#st-wall-bg)" rx="4" class="showtime-led-glow" stroke="#1a1a1a" stroke-width="3"/>
					<!-- Inner content zone -->
					<rect x="240" y="80" width="720" height="470" fill="rgba(10,2,2,0.8)" rx="2"/>
					<!-- Top truss -->
					<rect x="140" y="50" width="920" height="22" fill="#1e1e1e" rx="2" stroke="#2a2a2a" stroke-width="1"/>
					{#each Array.from({ length: 23 }, (_, i) => i) as i}
						<line x1={140 + i * 40} y1="50" x2={140 + (i + 1) * 40} y2="72" stroke="#2a2a2a" stroke-width="1.5"/>
						<line x1={140 + i * 40} y1="72" x2={140 + (i + 1) * 40} y2="50" stroke="#2a2a2a" stroke-width="1.5"/>
					{/each}
					<!-- Par lights on truss -->
					...
					{#each Array.from({ length: 12 }, (_, i) => i) as i}
						<line x1="140" y1={50 + i * 40} x2="154" y2={50 + (i+1) * 40} stroke="#2a2a2a" stroke-width="1.5"/>
						<line x1="154" y1={50 + i * 40} x2="140" y2={50 + (i+1) * 40} stroke="#2a2a2a" stroke-width="1.5"/>
					{/each}
					<rect x="1046" y="50" width="14" height="480" fill="#1e1e1e" rx="2"/>
					{#each Array.from({ length: 12 }, (_, i) => i) as i}
						<line x1="1046" y1={50 + i * 40} x2="1060" y2={50 + (i+1) * 40} stroke="#2a2a2a" stroke-width="1.5"/>
						<line x1="1060" y1={50 + i * 40} x2="1046" y2={50 + (i+1) * 40} stroke="#2a2a2a" stroke-width="1.5"/>
					{/each}
				</svg>
			</div>
			<!-- Spotlight sweep -->
			...
			<canvas bind:this={confettiCanvas} class="absolute inset-0 h-full w-full pointer-events-none" style="z-index:5"></canvas>
			<!-- Flame throwers -->
