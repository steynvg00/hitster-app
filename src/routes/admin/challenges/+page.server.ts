...
export const actions: Actions = {
	createChallenge: async ({ request, locals }) => {
		...
		const type = (data.get('type') as string)?.trim() as ChallengeType;
		const title = (data.get('title') as string)?.trim() || `New ${type} challenge`;

		if (!type || !(CHALLENGE_TYPES as readonly string[]).includes(type)) {
		...
		const { data: created, error } = await db
			...
			.insert({
				...
				timer_seconds: 60,
				points_config: { field_points: fieldPoints } as never,
				status: 'draft',
				is_active: false,
