<script lang="ts">
	...
	const isPodiumPhase = $derived(revealIndex >= totalTeams - Math.min(3, totalTeams) && revealIndex < totalTeams);
...
<div class="min-h-screen p-6 max-w-5xl mx-auto">
	...
	<div class="mb-6 flex items-start justify-between gap-4">
		...
		<div class="flex gap-2 shrink-0">
			{#if recapState !== 'complete'}
				{#if !allRevealed}
					<form method="POST" action="?/reveal" use:enhance={({ cancel }) => {
						return async ({ update }) => { await update({ reset: false }); };
					}}>
						<button type="submit"
							class="rounded-lg bg-amber-400 px-4 py-2 text-sm font-bold text-zinc-950 hover:bg-amber-300 transition-colors">
							Reveal next ›
						</button>
					</form>
				{:else}
					<form method="POST" action="?/endAndReset" use:enhance
						onsubmit={(e) => { if (!confirm('End set and clear all player assignments?')) e.preventDefault(); }}>
						<button type="submit"
							class="rounded-lg bg-green-700 px-4 py-2 text-sm font-bold text-white hover:bg-green-600 transition-colors">
							End & Reset
						</button>
					</form>
				{/if}
			...
			<!-- TV podium link -->
			<a href="/sets/{gs.id}/podium" target="_blank"
	...
	{#if allRevealed && recapState !== 'complete'}
		<div class="mt-8 rounded-xl border border-green-800 bg-green-950/30 p-6 text-center">
			<p class="text-lg font-bold text-green-400 mb-1">All teams revealed!</p>
			<p class="text-sm text-zinc-400">Click "End & Reset" to finish the set and send players to their thanks screen.</p>
		</div>
	{/if}
</div>
