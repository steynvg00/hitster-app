...
<div class="flex h-full flex-col">
	...
	<div class="flex items-center justify-between border-b border-zinc-800 bg-zinc-900 px-6 py-4">
		...
		<div class="flex items-center gap-3">
			...
			<form
				...
				use:enhance
				onsubmit={(e) => {
					if (!confirm('Clear all player assignments for this set?')) e.preventDefault();
				}}
			>
				<button
					class="rounded-lg border border-zinc-700 px-3 py-1.5 text-sm font-medium text-zinc-400 transition-colors hover:border-zinc-500 hover:text-white"
				>
					Reset All
				</button>
			</form>
	...
	{#if form?.error}
		<div
			class="mx-6 mt-4 rounded-lg border border-red-800 bg-red-950/40 px-4 py-2 text-sm text-red-400"
		>
			{form.error}
		</div>
	{/if}
	...
	<div class="flex-1 overflow-auto p-6">
		<div
			class="grid gap-4"
			style="grid-template-columns: repeat({data.teams.length}, minmax(0, 1fr))"
		>
			{#each data.teams as team}
				{@const players = playersByTeam[team.id] ?? []}
				<div
					class="overflow-hidden rounded-xl border bg-zinc-900"
					style="border-color: {TEAM_COLOR_BORDER[team.color] ?? '#3f3f46'}"
				>
					<!-- Team header -->
					<div
						class="px-4 py-3 text-sm font-bold tracking-wide text-white uppercase"
					...
					<!-- Player list -->
					<div class="min-h-16 divide-y divide-zinc-800">
						{#each players as p}
							<div class="flex items-center gap-2 px-3 py-2">
								...
								{:else}
									<div
										class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-zinc-700 text-xs font-black text-zinc-400"
									>
								...
								<button
									...
									class="shrink-0 rounded p-0.5 text-xs text-zinc-600 transition-colors hover:text-zinc-300"
									title="Move player"
								>
									⇄
								</button>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
		<div class="w-80 rounded-2xl border border-zinc-700 bg-zinc-900 p-5 shadow-2xl">
				...
				{#each data.teams as t}
					...
					>
						...
						<button
							...
							style="border-color: {TEAM_COLOR_BORDER[t.color] ??
								'#3f3f46'}; background-color: {TEAM_COLOR_BG[t.color] ?? '#27272a'}"
						>
							{t.display_name}
						</button>
					</form>
			...
			<button
				...
				class="mt-4 w-full rounded-lg border border-zinc-700 py-2 text-sm text-zinc-400 transition-colors hover:text-white"
			>
				Cancel
			</button>
		</div>
