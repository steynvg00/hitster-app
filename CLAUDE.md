...
| 2026-08-28 | Strafkaart + logo-cache (fix/strafkaart-en-logo): de strafkaart verdween op het VERSE pad doordat `use:enhance` eerst `invalidateAll()` en pas daarna `applyAction()` doet — tussen die twee is `data.priorResult` al gevuld en `earnedPowerups` nog niet, dus de instapfase werd op een lege strafwachtrij vastgelegd en `nextPhase` loopt alleen vooruit. Nieuw: `freshEntry()` in $lib/result-flow (wacht op `!submitting` én een lege wachtkamer, alle argumenten onvoorwaardelijk gelezen); `submitting` loopt af in een `finally`. De resume-bron uit 0082 stond achter een eenmalige seed-vlag en was daarmee een momentopname bij het mounten — nu herhaalbaar met dedup op `teamPowerupId` (`straffenInBeeldGeweest`, bewust een kale Set). Geen migratie nodig. `vercel.json`: /uploads/\* van `max-age=604800` naar `max-age=300, must-revalidate` — hernoemen naar -v3 is geen cachestrategie, en content-hashing raakt de host-geüploade gamesetlogo's en het runtime-pad van `powerupIcon()` niet. Gemeten op de gedeployde URL: het logo-asset is schoon (alfa 1-4 = 0,00%, hoeken op 0, sha256 gelijk aan de repo). |
